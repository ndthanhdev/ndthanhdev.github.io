...
My website is built with [Gatsby](https://www.gatsbyjs.com/), a popular tool for creating static websites. I have scripts in the scripts/actions folder that handle checking code quality, building the website, and deploying it to different environments.
...
Getting the website live involved building everything and then publishing it. To make this process smooth, I created scripts in a folder called `scripts/pipelines`. These scripts simply run the `build` and `publish` functions in the right order, one after the other.
